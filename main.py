...
# ========= ENV =========
BYBIT_BASE       = os.getenv("BYBIT_BASE", "https://api.bybit.com")        # oder Testnet: https://api-testnet.bybit.com
BYBIT_KEY        = os.getenv("BYBIT_KEY", "")
BYBIT_SECRET     = os.getenv("BYBIT_SECRET", "")
SETTLE_COIN      = os.getenv("SETTLE_COIN", "USDT")                        # linear-perps: USDT / USDC

ALLOWED_TFS      = set(os.getenv("ALLOWED_TFS", "M5,M15,H1").replace(" ", "").split(","))

# TP Splits für 3 TPs (Summe <=100; Rest = trailing-Anteil)
TP_SPLITS_THREE  = os.getenv("TP_SPLITS_THREE", "30,30,30").replace(" ", "")
MAX_LEV_CAP      = int(os.getenv("MAX_LEV_CAP", "75"))
SAFETY_PCT       = float(os.getenv("SAFETY_PCT", "80"))                    # lev = floor(SAFETY_PCT / SL%)

COOLDOWN_MIN     = int(os.getenv("COOLDOWN_MIN", "30"))                    # Minuten Cooldown NACH Fill
ENTRY_EXP_MIN    = int(os.getenv("ENTRY_EXP_MIN", "60"))                   # Entry-GTC wird nach X min gecancelt
DD_LIMIT_PCT     = float(os.getenv("DD_LIMIT_PCT", "2.8"))                 # optional, aktuell nicht hart erzwungen

MAX_OPEN_LONGS   = int(os.getenv("MAX_OPEN_LONGS", "999"))                 # limitierte Anzahl gefüllter Long-Positionen
MAX_OPEN_SHORTS  = int(os.getenv("MAX_OPEN_SHORTS", "999"))

TEXT_PATH        = os.getenv("TEXT_PATH", "content")                       # falls dein Webhook-JSON Text woanders trägt
DEFAULT_NOTIONAL = float(os.getenv("DEFAULT_NOTIONAL", "50"))              # Margin pro Trade (ohne Hebel)

# Trailing-Konfiguration (startet NUR nach TP3-Fill auf Rest-Bestand)
TRAIL_ENABLE     = os.getenv("TRAIL_ENABLE", "on").lower() == "on"
TRAIL_PCT        = float(os.getenv("TRAIL_PCT", "1.5"))                    # % Abstand (Callback) des Trailing-SL
TRAIL_POLL_SEC   = float(os.getenv("TRAIL_POLL_SEC", "3"))                 # Poll für Trailing
...
STATE: Dict[str, Any] = {
    "last_trade_ts": 0.0,             # wird bei (Teil-)Fill gesetzt → Cooldown
...
# ========= Parse TP splits =========
def parse_tp_splits_three(raw: str) -> Tuple[float,float,float,float]:
    """
    Erwartet "a,b,c". Summe <= 100. Rest = trailing.
    Returns (p1, p2, p3, trail_rest)
    """
    try:
        ...
        s = sum(parts)
        if s > 100.0:
            # scale down proportional, trailing bleibt 0
            parts = [p * (100.0/s) for p in parts]
            s = 100.0
        trail = max(0.0, 100.0 - s)
        return parts[0], parts[1], parts[2], trail
    except Exception:
        # Fallback 30/30/30 → 10% trail
        return 30.0, 30.0, 30.0, 10.0

TP1_POS_PCT, TP2_POS_PCT, TP3_POS_PCT, TRAIL_POS_PCT = parse_tp_splits_three(TP_SPLITS_THREE)
...
async def get_symbol_meta(symbol: str) -> Dict[str, float]:
    """
    Fetch & cache tickSize (price), stepSize (qty), minOrderQty, minNotional for linear perps.
    """
...
async def get_last_price(symbol: str) -> float:
    res = await bybit("/v5/market/tickers", "GET", {
        "category": "linear",
        "symbol": symbol
    })
    lst = res.get("list", [])
    if not lst: 
        raise HTTPException(502, f"No ticker for {symbol}")
    return float(lst[0]["lastPrice"])

async def set_stop(symbol: str, price: float):
    meta = await get_symbol_meta(symbol)
    px = quant_price(price, meta["tickSize"])
    await bybit("/v5/position/set-trading-stop","POST",{
        "category":"linear","symbol":symbol,"tpSlMode":"Full","stopLoss": str(px)
    })
    print(f"🔧 SL update {symbol} -> {px}")

# ========= Parser (Zeiierman-Style mit Timeframe) =========
def parse_signals(text: str) -> List[Dict[str, Any]]:
    ...
    for b in blocks:
        ...
        m_tp3  = re.search(r"TP\s*3:\s*([0-9]*\.?[0-9]+)", b, re.I)  # optional
        ...
        tp3   = float(m_tp3.group(1)) if m_tp3 else None
        sl    = float(m_sl.group(1))

        # Plausibilität
        if side=="long":
            if not (sl < entry < tp1 <= tp2 and (tp3 is None or tp2 <= tp3)):
                continue
        else:
            if not (sl > entry > tp1 >= tp2 and (tp3 is None or tp2 >= tp3)):
                continue
        ...
        signals.append({
            "base":base,"quote":quote,"side":side,"entry":entry,"tp1":tp1,"tp2":tp2,"tp3":tp3,"sl":sl,"tf":tf
        })
    return signals
...
async def place_tp_sl_three(symbol: str, side: str, size: float, tp1: float, tp2: float, tp3: Optional[float], sl: float) -> Tuple[List[Optional[str]], str]:
    """
    Legt TP1/TP2/TP3 als reduceOnly-Limits an und den StopLoss als positionsweiten SL.
    Trailing erfolgt später synthetisch nach TP3-Fill.
    """
    meta = await get_symbol_meta(symbol)
    tick = meta["tickSize"]; step = meta["stepSize"]; minQty = meta["minQty"]
    ...
    tp3_q = quant_price(tp3, tick) if tp3 is not None else None
    sl_q  = quant_price(sl,  tick)

    # Split sizes (Rest bleibt im Markt -> trailing später)
    q1_raw = size * (TP1_POS_PCT/100.0)
    q2_raw = size * (TP2_POS_PCT/100.0)
    q3_raw = size * (TP3_POS_PCT/100.0)

    q1 = quant_qty(max(q1_raw, 0.0), step)
    q2 = quant_qty(max(q2_raw, 0.0), step)
    q3 = quant_qty(max(q3_raw, 0.0), step)

    # Sicherstellen, dass Summe <= size
    total = q1 + q2 + q3
    if total > size:
        over = total - size
        # reduziere q3 zuerst, dann q2
        take = min(q3, over); q3 -= take; over -= take
        if over > 0:
            take = min(q2, over); q2 -= take; over -= take
        if over > 0:
            q1 = quant_qty(max(q1 - over, 0.0), step)

    OP = "Sell" if side=="long" else "Buy"
    uid = hex(int(now_ts()*1000))[2:]
    tp_links: List[Optional[str]] = [None, None, None]
    ...
    if q1 >= minQty:
        ...
        print(f"ℹ️ TP1 skipped (q1<{minQty})")
    ...
    # TP3
    if tp3_q is not None and q3 >= minQty:
        link_tp3 = f"tp3_{symbol}_{uid}"
        await bybit("/v5/order/create","POST",{
            "category":"linear","symbol":symbol,"side":OP,
            "orderType":"Limit","price":str(tp3_q),"qty":str(q3),
            "timeInForce":"GTC","reduceOnly":True,"closeOnTrigger":False,
            "orderLinkId":link_tp3
        })
        tp_links[2] = link_tp3
        print(f"✅ TP3 {symbol} {OP} {q3} @ {tp3_q}")
    else:
        print(f"ℹ️ TP3 skipped (q3<{minQty} oder kein TP3 im Signal)")
    ...
    return tp_links, "pos_stop"

async def move_sl_to_be(symbol: str, side: str, entry_price: float):
    meta = await get_symbol_meta(symbol)
    be_q  = quant_price(entry_price, meta["tickSize"])
    # winziger Offset, damit er nicht sofort greift
    off   = max(be_q * 0.0002, meta["tickSize"])  # ~2bp
...
async def monitor_loop():
    ...
    while True:
        ...
        for symbol, meta in list(STATE["open_watch"].items()):
            try:
                # Phase A: warten bis Position (teil)gefüllt -> Exits anlegen
                if not meta.get("exits_set"):
                    ...
                    if size > 0:
                        tp_links, sl_id = await place_tp_sl_three(
                            symbol, meta["side"], size,
                            meta["tp1_px"], meta["tp2_px"], meta["tp3_px"], meta["sl_px"]
                        )
                        meta.update({
                            "tp_links": tp_links, "sl_id": sl_id,
                            "exits_set": True, "tp1_hit": False,
                            "sl_be": False, "trail_active": False
                        })
                        STATE["last_trade_ts"] = now_ts()  # Cooldown erst jetzt
                        continue

                # Phase B: nach TP1 -> SL zu BE
                if meta.get("exits_set") and not meta.get("tp1_hit"):
                    tp1 = (meta.get("tp_links") or [None])[0]
                    if tp1:
                        st_tp1 = await get_order_status(symbol, tp1)
                        if st_tp1 == "Filled":
                            meta["tp1_hit"] = True
                ...
                # Phase B2: Trailing starten nach TP3-Fill
                if TRAIL_ENABLE and meta.get("exits_set") and not meta.get("trail_active"):
                    tp_links = meta.get("tp_links", [])
                    tp3 = tp_links[2] if len(tp_links) >= 3 else None
                    if tp3:
                        st_tp3 = await get_order_status(symbol, tp3)
                        if st_tp3 == "Filled":
                            cur_px = await get_last_price(symbol)
                            if meta["side"] == "long":
                                meta["trail_peak"] = cur_px
                                meta["trail_trough"] = None
                            else:
                                meta["trail_trough"] = cur_px
                                meta["trail_peak"] = None
                            meta["trail_active"] = True
                            meta["last_trail_update"] = 0.0
                            print(f"🏁 TP3 hit -> start trailing {symbol} ({TRAIL_PCT}%)")

                # Phase B3: aktives Trailing – SL nachziehen
                if TRAIL_ENABLE and meta.get("trail_active"):
                    if now_ts() - meta.get("last_trail_update", 0) >= max(1.0, TRAIL_POLL_SEC):
                        meta["last_trail_update"] = now_ts()
                        cur_px = await get_last_price(symbol)
                        side = meta["side"]
                        pct  = TRAIL_PCT / 100.0

                        if side == "long":
                            if cur_px > (meta.get("trail_peak") or -1):
                                meta["trail_peak"] = cur_px
                            peak = meta["trail_peak"] or cur_px
                            new_sl = peak * (1 - pct)
                        else:
                            if cur_px < (meta.get("trail_trough") or 1e99):
                                meta["trail_trrough"] = cur_px  # typo guard
                                meta["trail_trough"] = cur_px
                            trough = meta.get("trail_trough") or cur_px
                            new_sl = trough * (1 + pct)

                        await set_stop(symbol, new_sl)

                # Phase C: Cleanup wenn Pos zu
...
async def webhook(request: Request):
    ...
    entry, tp1, tp2, tp3, sl = sig["entry"], sig["tp1"], sig["tp2"], sig.get("tp3"), sig["sl"]
    symbol = f"{base}{quote}"

    print(f"📨 Signal: {symbol} {side} Entry:{entry} TP1:{tp1} TP2:{tp2} TP3:{tp3} SL:{sl}")
    ...
    STATE["open_watch"][symbol] = {
        ...
        "entry_px": entry, "tp1_px": tp1, "tp2_px": tp2, "tp3_px": tp3, "sl_px": sl,
        "created_ts": now_ts(), "expiry_min": ENTRY_EXP_MIN,
        "exits_set": False, "tp1_hit": False, "sl_be": False,
        "trail_active": False, "tp_links": [None, None, None]
    }
    ...
    return {
        ...
        "open_counts": counts,
        "tp_splits": {
            "tp1_pct": TP1_POS_PCT, "tp2_pct": TP2_POS_PCT, "tp3_pct": TP3_POS_PCT, "trail_rest_pct": TRAIL_POS_PCT
        },
        "trailing": {"enabled": TRAIL_ENABLE, "callback_pct": TRAIL_PCT}
    }
